    ...
    <div class="container">
        ...
        <div class="template-container">
            ...
            <form method="post" action="{{ route('savePatientNotes') }}">
                ...
                <?php $title = 0; ?>
                <!-- First Iterate to display title -->
                @foreach ($data as $titleContent)
                    ...
                    @if (array_key_exists('headings', $titleContent))
                        @foreach ($titleContent['headings'] as $headingTitle)
                            ...
                            <!-- Check if it has content itself instead of sub headings -->
                            @if (isset($headingTitle['content']))
                                <!-- Check if the content is text input instead of select -->
                                @if ($headingTitle['content'] == 'InputText')
                                    <!-- check content is not speech to text should be on right not as new row -->
                                    <!-- Creating a new row for input text -->
                                    <div class="row">
                                        <div class="col-lg-5">
                                            <!-- Check if the text input is for comments -->
                                            @if ($headingTitle['heading'] == 'Comments:')
                                                ...
                                                <input title="{{ $titleContent['title'] }}" content="InputText"
                                                    ...
                                                    value="{{ $headingTitle['value'] }}" typeVal="InputText"
                                                    class="templateFormInput" id="myData" type="text"
                                                    name="" placeholder="Insert Comments">
                                            @elseif ($headingTitle['heading'] != 'Speech to text')
                                                <!-- Text inputs for normal questions instead of comments -->
                                                <input title="{{ $titleContent['title'] }}" content="InputText"
                                                    uniqName="{{ $headingId }}" dataType="comments"
                                                    question="{{ $headingTitle['heading'] }}"
                                                    value="{{ $headingTitle['value'] }}" typeVal="InputText"
                                        ...
                                        <div class="col-lg-6 col-md-6 col-xs-10 mt-5">
                                            <!-- Text input for speech to text -->
                                            <input title="{{ $titleContent['title'] }}" content="InputText"
                                                uniqName="{{ $headingId }}" question="Speech to text"
                                                typeVal="speechToText" class="templateFormInput"
                                                value="{{ $headingTitle['speechToTextValue'] }}" id="output"
                                                type="text" name="" placeholder="Speech to Text">
                                        </div>
                                    </div>
                                    <br />
                                @else
                                    <!-- If the input is select type -->
                                    <div class="row">
                                        <div class="col-lg-5">
                                            <!-- Multiple select Input -->
                                            <select title="{{ $titleContent['title'] }}"
                                                ...
                                                @if ($headingTitle['value'] != '')
                                                    @foreach ((array) $headingTitle['value'] as $selectedContent)
                                                        <option value="{{ $selectedContent }}" selected>
                                                            {{ $selectedContent }}</option>
                                                    @endforeach
                                                @endif
                                                @foreach (explode(',', $headingTitle['content']) as $content)
                                        ...
                                        <div class="col-lg-6 col-md-6 col-xs-10 mt-5">
                                            ...
                                            <input title="{{ $titleContent['title'] }}" content="InputText"
                                                uniqName="{{ $headingId }}" question="Speech to text"
                                                typeVal="speechToText" class="templateFormInput speechToText"
                                                value="{{ $headingTitle['speechToTextValue'] }}" id="output"
                                                type="text" name="" placeholder="Speech to Text">
                                        </div>
                                    </div>
                                    ...
                                    <div class="row">
                                        <div class="col-lg-5">
                                            ...
                                            <select title="{{ $titleContent['title'] }}"
                                                ...
                                                id="myData" aria-label=".form-select-lg example">
                                                @if ($subHeading['value'] != '')
                                                    @foreach ((array) $subHeading['value'] as $selectedContent)
                                                        <option value="{{ $selectedContent }}" selected>
                                                            {{ $selectedContent }}</option>
                                                    @endforeach
                                                @endif
                                                @foreach (explode(',', $subHeading['content']) as $content)
                                                    <option value="{{ $content }}">{{ $content }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        ...
                                        <div class="col-lg-6 col-md-6 col-xs-10 mt-5">
                                            ...
                                            <input title="{{ $titleContent['title'] }}"
                                                uniqName="{{ $headingId }}"
                                                value="{{ $subHeading['speechToTextValue'] }}"
