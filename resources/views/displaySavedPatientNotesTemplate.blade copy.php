               ...
               <div class="elementor-widget-wrap elementor-element-populated">
                   <div class="elementor-element elementor-element-2ab8f29 elementor-widget elementor-widget-image" data-id="2ab8f29" data-element_type="widget" data-widget_type="image.default">
                       <div class="elementor-widget-container" style="display: flex;
                           ...
                           </style><a href="{{ route('dashboard') }}"> <img decoding="async" width="300" height="57" src="../images/2023-02-cropped-cropped-DIAN--300x57.png" class="attachment-medium size-medium wp-image-105" alt="" loading="lazy" /></a>
                       </div>
                   </div>
               </div>
           </div>
   ...
   <div class="elementor-element elementor-element-f9d95f1 elementor-widget-divider--view-line elementor-widget elementor-widget-divider" data-id="f9d95f1" data-element_type="widget" data-widget_type="divider.default">
    ...
    <form method="post" action="{{ route('savePatientNotes') }}">
        ...
        @foreach($data as $titleContent)
            ...
            $title = $title + 1;
            $titleId = "t" . $title; ?>
            <input question="{{ $titleContent['title'] }}" dataType="title" uniqName="{{ $titleId }}" title="{{ $titleContent['title'] }}" type="hidden">
            ...
            <?php if (array_key_exists("headings",$titleContent)) {?>
                ...
                @foreach($titleContent['headings'] as $headingTitle)
                    ...
                    <?php if (isset($headingTitle['heading'])) { ?>
                        @if($headingTitle['heading'] != 'Speech to text')
                    ...
                    <?php if (isset($headingTitle['content'])) { ?>
                        <?php if($headingTitle['content'] == 'InputText'){ ?>
                            ...
                            <div class="row">
                                ...
                                <input title="{{ $titleContent['title'] }}" content="InputText" uniqName="{{ $headingId }}" dataType="comments" question="{{ $headingTitle['heading'] }}" value="{{ $headingTitle['value'] }}" typeVal = "InputText" class="templateFormInput" id="myData" type="text" name="" placeholder="Insert Comments">
                                ...
                                <?php $count = $count + 1; ?>
                                @if($headingTitle['heading'] == 'Comments:')
                                <input title="{{ $titleContent['title'] }}" content="InputText" uniqName="{{ $headingId }}" dataType="lastHeading" question="Speech to text"  typeVal = "speechToText" value="{{ $headingTitle['value'] }}" class="templateFormInput" id='output' type="text" name="" placeholder="Speech to Text">
                                @else
                                <input title="{{ $titleContent['title'] }}" content="InputText" uniqName="{{ $headingId }}" question="Speech to text"  typeVal = "speechToText" class="templateFormInput" value="{{ $headingTitle['value'] }}" id='output' type="text" name="" placeholder="Speech to Text">
                                @endif
                            </div>
                            @endif
                        </div>
                        ...
                        <?php } else { ?>
                            <div class="row">
                                <div class="col-lg-5">
                                    ...
                                    $selectContent = $headingTitle['content']; ?>
                                <select title="{{ $titleContent['title'] }}" content="{{ $selectContent }}" uniqName="{{ $headingId }}" dataType="content" multiple question="{{ isset($headingTitle['heading']) ? $headingTitle['heading'] : '' }}" class="chosen-select templateFormSelect form-select form-select-lg mb-3" id="myData" aria-label=".form-select-lg example">
                                
                                @if($headingTitle['value'] != '')

                                    @foreach((array)($headingTitle['value']) as $selectedContent)
                                    <option value="{{ $selectedContent }}" selected>{{ $selectedContent }}</option>
                                    @endforeach
                                    
                                @endif
                                @foreach((array)($headingTitleContentArray) as $content)
                                ...
                                <div class="col-lg-6">
                                <?php $count = $count + 1; ?>
                                <input title="{{ $titleContent['title'] }}" content="InputText"  uniqName="{{ $headingId }}" question="Speech to text"  typeVal = "speechToText" class="templateFormInput speechToText" id='output' type="text" name="" placeholder="Speech to Text">
                                </div>
                            </div>
        ...
        <input type="button" id="generateJsonText" onclick="generateJsonText()" class="copyTextBtn" value="Save">
